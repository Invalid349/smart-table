...
export function initSorting(columns) {
    return (data, state, action) => {
        ...
        if (action && action.name === 'sort') {
            ...
            field = action.dataset.field; // Получаем имя поля из data-field кнопки
            const currentValue = action.dataset.value; // Получаем текущее значение сортировки из data-value
            order = sortMap[currentValue]; // Получаем следующее значение из карты переходов
            action.dataset.value = order; // Обновляем data-value кнопки на новое значение
